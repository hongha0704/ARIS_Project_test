...
with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    while True:
        ...
        # 현재 프레임 예측 (YOLO)
        boxes, masks, cls, probs = predict_on_image(model, frame, conf=0.80)

        # BGR 이미지를 RGB로 변환 (MediaPipe 용)
        ...
        image_with_masks = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR)
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                for landmark in hand_landmarks.landmark:
                    ...
                    human_points.append([cx, cy])
        
        # 최단 거리 계산 및 시각화
        if robot_contours and human_points:
            robot_points = np.vstack(robot_contours).squeeze()
            human_points = np.array(human_points, dtype=np.float32)
            ...
            cv2.line(image_with_masks, robot_point, human_point, (255, 255, 255), 2)
