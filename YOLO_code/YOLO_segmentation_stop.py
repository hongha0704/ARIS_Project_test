...
# # 모델 로드
# model = YOLO('/home/beakhongha/YOLO_ARIS/train15/weights/best.pt')
...
#     # 현재 프레임 예측
#     boxes, masks, cls, probs = predict_on_image(model, rgb_frame, conf=0.85)
...
while True:
    ...
    # 현재 프레임 예측
    boxes, masks, cls, probs = predict_on_image(model, rgb_frame, conf=0.3)
    ...
    # 최단 거리 계산 및 시각화
    if robot_contours and hand_contours:
        robot_points = np.vstack(robot_contours).squeeze()
        hand_points = np.vstack(hand_contours).squeeze()
        dists = cdist(robot_points, hand_points)
        min_dist_idx = np.unravel_index(np.argmin(dists), dists.shape)
        robot_point = robot_points[min_dist_idx[0]]
        hand_point = hand_points[min_dist_idx[1]]
        min_distance = dists[min_dist_idx]

        # 최단 거리 표시
        cv2.line(image_with_masks, tuple(robot_point), tuple(hand_point), (255, 255, 255), 2)
        mid_point = ((robot_point[0] + hand_point[0]) // 2, (robot_point[1] + hand_point[1]) // 2)
        cv2.putText(image_with_masks, f'{min_distance:.2f}', mid_point, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
