...
class RobotMain(object):
    ...
    def run_test(self):
        ...
        # # mode = 1
        # code = self._arm.set_servo_angle_j(angles=[179.5, 33.5, 32.7, 113.0, 93.1, -2.3], speed=self._angle_speed,
        #                                      mvacc=self._angle_acc)
        # print(f"Return code: {code}")
        
        # mode = 4
        self._is_set_move = True
        speeds = [0, 0, 0, 0, 0, 10, 0]  # 속도 값은 각 관절의 속도를 나타냅니다.
        self._arm.vc_set_joint_velocity(speeds, is_radian=False, duration=2)
        # print(f"Return code: {code}")

        # # dh_params
        # dh_params = self._arm.get_dh_params()
        # print(f"dh_params: {dh_params}")

        # get_inverse_kinematics
        pose = [-257.3, -138.3, 198, 68.3, 86.1, -47.0]
        code, angles = self._arm.get_inverse_kinematics(pose, input_is_radian=False, return_is_radian=False)
        print(f'angles = {angles}')

        # # get_forward_kinematics
        # angles = [179.2, -42.1, 7.4, 186.7, 41.5, -1.6]
        # code, pose = self._arm.get_forward_kinematics(angles, input_is_radian=False, return_is_radian=False)
        # print(f'pose = {pose}')

        print('run_test finish')
