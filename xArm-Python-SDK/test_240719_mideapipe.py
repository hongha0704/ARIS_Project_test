...
class RobotMain(object):
    ...
    def run_test(self):
        ...
        while(1):
            # 각도 제한
            angle_shoulders = np.clip(angle_shoulders, -80, 80)
            angle_shoulder_elbow = np.clip(angle_shoulder_elbow, 60, 300)
            #angle_elbow_wrist = np.clip(angle_shoulders, -90, 90)

            code = self._arm.set_servo_angle_j(angles=[179.2, angle_shoulders, angle_shoulder_elbow, 186.7, angle_elbow_wrist, 90], speed=self._angle_speed,
                                            mvacc=self._angle_acc)
            
            print(f'move to : [179.200, {angle_shoulders:.3f}, {angle_shoulder_elbow:.3f}, 186.700, {angle_elbow_wrist:.3f}, -1.600]')
            time.sleep(0.3)
