...
class YOLOMain:
    def object_dectection(self):
        global A, B, C, D
        # Load a model
        model = YOLO("/home/beakhongha/YOLO_ARIS/train5/weights/best.pt")
        ...
        # 영구적으로 설정된 ROI 구역
        rois = [(480, 140, 92, 92), (388, 140, 92, 92), (296, 140, 92, 92)]  # A, B, C 순서
        specific_roi = (491, 302, 116, 119)  # 특정 ROI 구역

        # 변수 초기화
        A = False  # 첫 번째 ROI 내에서 capsule 객체가 인식되었는지 여부
        B = False  # 두 번째 ROI 내에서 capsule 객체가 인식되었는지 여부
        C = False  # 세 번째 ROI 내에서 capsule 객체가 인식되었는지 여부
        D = False  # 특정 ROI 내에서 capsule_not_label 객체가 인식되었는지 여부
