...
class YOLOMain:
    def __init__(self, robot_main):
        # 모델 로드
        self.model = YOLO('/home/beakhongha/YOLO_ARIS/train21/weights/best.pt')
    ...
    def segmentation(self):
        ...
        while True:
            ...
            for box, mask, class_id, prob in zip(boxes, masks, cls, probs):  # 각 객체에 대해
                ...
                if label == 'cup':  # 'cup' 객체에 대해서만 중심 좌표 계산 및 출력
                    ...
                    if last_cup_center is not None:
                        if (center_x_mm is not None and center_y_mm is not None and                               # 중심좌표가 존재하면
                            ...
                            elif current_time - trash_detect_start_time >= 1:  # 1초 이상 ROI 내에 존재하고 중심 좌표 변경 없을 시 쓰레기 탐지
                                cup_trash_detected = True
                            else:
                                # 탐지 시작하고 0초부터 2초까지 단위로 출력
                                print(f"Cup detected for {current_time - trash_detect_start_time:.2f} seconds")
                        else:
                ...
                # 'human'의 디텍션 박스가 픽셀 기준 95이상에 존재할 때만 최단 거리 계산 및 로봇 일시정지 기능 활성
                if label == 'human':
                    human_y2 = y2

            # 최단 거리 계산 및 시각화
            if robot_contours and human_contours and human_y2 > 95:
...
class RobotMain(object):
    ...
    def trash_mode(self):
        ...
        if center_x_mm > 100 and center_x_mm < 380 and center_y_mm >= -130 and center_y_mm <= 100:
            ...
            code = self._arm.set_position(*[center_x_mm-20, -173, 307.5, -173, 13.3, -87.6], speed=self._tcp_speed,
                                        mvacc=self._tcp_acc, radius=0.0, wait=True)
            if not self._check_code(code, 'set_position'): return
