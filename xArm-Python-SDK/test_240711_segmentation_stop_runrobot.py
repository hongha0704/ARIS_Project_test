...
class YOLOMain:
    def __init__(self):
        # 모델 로드
        self.model = YOLO('/home/beakhongha/YOLO_ARIS/train7/weights/best.pt')
    ...
    def segmentation(self):
        ...
        while True:
            ...
            # 화면 왼쪽 위에 최단 거리 표시
            cv2.putText(image_with_masks, f'Distance: {min_distance:.2f}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            # 거리 조건 체크 및 로봇 제어
            if min_distance <= 30 and min_distance_bool:
                if robot_paused.is_set():
                    print(f'state : stop!, min_distance = {min_distance}')
                    robot_paused.clear()
            elif min_distance > 30 or not min_distance_bool:
                if not robot_paused.is_set():
